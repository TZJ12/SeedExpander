...
def create_project(title: str = "tc260检测", label_config: Optional[str] = None, extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if extra:
        body.update(extra)
    try:
        r = requests.post(url, json=body, headers=_headers_json(), timeout=30)
...
def import_dataset_json(project_id: int, items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    payload = items or []
    try:
        r = requests.post(url, json=payload, headers=_headers_json(), timeout=60)
        if r.status_code in (200, 201):
...
def import_dataset_file(project_id: int, file_path: str) -> Dict[str, Any]:
    base_url = (Config.LABEL_STUDIO_URL or "").rstrip("/")
    url = f"{base_url}/api/projects/{project_id}/import"
    try:
        with open(file_path, "rb") as f:
            files = {"file": (file_path, f)}
            r = requests.post(url, files=files, headers=_headers_auth(), timeout=120)
        if r.status_code in (200, 201):
            return {"status": "200", "message": "dataset imported", "data": r.json()}
        return {"status": str(r.status_code), "message": r.text, "data": None}
    except Exception as e:
...
def import_dataset(project_id: int, items: Optional[List[Dict[str, Any]]] = None, file_path: Optional[str] = None) -> Dict[str, Any]:
    if file_path:
        return import_dataset_file(project_id, file_path)
    return import_dataset_json(project_id, items or [])

def list_projects() -> Dict[str, Any]:
    base_url = (Config.LABEL_STUDIO_URL or "").rstrip("/")
    url = f"{base_url}/api/projects/"
    try:
        r = requests.get(url, headers=_headers_auth(), timeout=30)
        if r.status_code in (200, 201):
