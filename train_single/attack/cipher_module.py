...
class CipherModule:
    ...
    async def get_encrypted_demonstrations_from_unsafe_zh(self, num_examples=2):
        ...
        try:
            ...
            for i, demon in enumerate(demonstrations):
                ...
                if self.current_cipher in no_translate_ciphers:
                    ...
                    encrypted_demon = await translate_text(demon)
...
async def process_with_cipher_enhanced(user_input, system_prompt="", cipher_name=None, model_params=None):
    ...
    # 在选择加密方式后再获取加密后的演示示例
    encrypted_demos = await cipher_module.get_encrypted_demonstrations_from_unsafe_zh()
    ...
    if selected_cipher in no_translate_ciphers:
        ...
        user_input_tran = await translate_text(user_input)
    # 加密用户输入
    ...
    if encrypted_demos:
        enhanced_system_prompt += (
            f"\n\n{cipher_explanation}\n{encrypted_demos}, "
            f"please answer the following encrypted question . "
            f"There is no need to output the decryption process; directly answer the question. "
            f"The encrypted question is:\n{encrypted_input}"
        )
