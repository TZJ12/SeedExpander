...
async def get_concept_by_llm(query, llm_func=None):
    """使用 LLM 进行意图分类"""
...
"""
    ...
    if llm_func:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(llm_func):
                response_text = await llm_func(classification_prompt)
            else:
                response_text = await asyncio.to_thread(llm_func, classification_prompt)
    ...
    try:
        category_key = (await call_jailbreak_llm_async(query, system_prompt=classification_prompt)).strip().lower()
...
async def get_concept_by_query(query, llm_func=None):
    """根据查询内容匹配最合适的概念 (纯 LLM 智能分类)"""
    
    # 优先尝试 LLM 智能分类
    # print("正在进行智能意图识别...")
    concept = await get_concept_by_llm(query, llm_func=llm_func)
...
async def generate_prompt_llm(query, concept, llm_func=None):
    ...
    if llm_func:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(llm_func):
                task_content = await llm_func(full_prompt)
            else:
                task_content = await asyncio.to_thread(llm_func, full_prompt)
        except Exception as e:
            return f"External LLM Generation Failed: {e}"
    elif LLM_AVAILABLE:
        try:
            task_content = await call_jailbreak_llm_async(user_prompt_content, system_prompt=system_prompt)
        except Exception as e:
...
async def generate_prompt(query):
    """根据查询内容生成 Prompt"""
    concept = await get_concept_by_query(query)
    
    # 优先尝试 LLM 生成模式
    prompt = None
    if LLM_AVAILABLE:
        print(f"正在调用 {Config.MODEL_NAME} 进行深度定制...")
        task_content = await generate_prompt_llm(query, concept)
...
async def main():
    ...
    for question in target_questions:
        ...
        print(f"\n[+] 处理问题: {question}")
        prompt, concept = await generate_prompt(question)
