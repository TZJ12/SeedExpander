...
def _ensure_llm_callable(llm_callable: Optional[Callable[[str], str]]) -> Callable[[str], str]:
    if llm_callable is not None:
        return llm_callable
    if call_jailbreak_llm is None:
        raise RuntimeError(
            "未提供可调用的 LLM，且 model.call_llm 不可用。"
        )
    return call_jailbreak_llm


def _generate(prompt: str, llm_callable: Callable[[str], str]) -> str:
    """调用模型并返回原始文本响应。"""
...
def transform_to_math_problem(
    ...
    - llm_callable: 可选，自定义模型调用函数；不传则使用 model.call_llm
    ...
    for _ in range(max_retries):
        ...
        raw_enhanced = _generate(prompt_enhance, llm)
        try:
        ...
        raw_compliance = _generate(prompt_compliance, llm)
        try:
        ...
        raw_is_math = _generate(prompt_is_math, llm)
        try:
...
def rewrite_json_questions(input_path: str, output_path: str):
    ...
    for category, items in data.items():
        for item in items:
            ...
            rewritten = transform_to_math_problem(question)
...
if __name__ == "__main__":
    ...
    reply = transform_to_math_problem(question)
    print(reply)
