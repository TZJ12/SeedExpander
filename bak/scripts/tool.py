...
async def _run_adaptive_core(
    ...
    def _save_intermediate_results(current_prob: float):
        # 结果分析逻辑 (仅分析成功越狱的)
    ...
    async def run_phase_tasks(phase_label: str, tpls: List[str], target_dids: List[int]) -> bool:
        ...
        BATCH_CONCURRENCY = 8 #批次并发数
        batch_sem = asyncio.Semaphore(BATCH_CONCURRENCY)
        POLL_CONCURRENCY = 48 #评估轮询并发数
        poll_sem = asyncio.Semaphore(POLL_CONCURRENCY)
        
        log_callback(f"[{phase_label}] Total pairs: {len(current_pairs)}. Splitting into batches of {BATCH_SIZE} (Max concurrent batches: {BATCH_CONCURRENCY})...")
        ...
        for i in range(0, len(current_pairs), BATCH_SIZE):
            ...
            batch_tasks.append(process_batch_task(batch_idx, chunk))
        ...
        for coro in asyncio.as_completed(batch_tasks):
            ...
            curr_prob_tmp = len(successful_pairs) / total_combinations
            
            log_callback(f"[{phase_label}] Batch {batch_idx} Finished. Matched {matched_count}/{len(details)} results. Eval Service Jail: {jb_cnt}. Total Successes: {len(successful_pairs)}")
            
            # Save Intermediate Results IMMEDIATELY
            _save_intermediate_results(curr_prob_tmp)
