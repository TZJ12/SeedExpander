...
# ==================== 模型接入配置 ====================
DEFAULT_API_URL = "http://0.0.0.0:8000/v1"
DEFAULT_API_KEY = "sk-390feb24-daee-11f0-a581-0242ac150003"
DEFAULT_MODEL_NAME = "oyster_1"
...
async def main():
    ...
    def _normalize_input(raw_input):
        """将输入统一为由字典组成的数组，兼容数组或对象两种格式。
        - 若为数组：过滤出字典元素
        - 若为对象：将各键的数组合并为一个数组，并在缺失时补充类别信息
        """
        if isinstance(raw_input, list):
            return [item for item in raw_input if isinstance(item, dict)]
        elif isinstance(raw_input, dict):
            normalized = []
            for top_key, val in raw_input.items():
                    ...
                    for item in val:
                        if isinstance(item, dict):
                            ...
                            if 'category' not in item:
                                item = {**item}
                                item.setdefault('category', top_key)
            ...
            raise ValueError("输入文件必须是JSON数组或JSON对象（键为类别，值为数组）")

    def _extract_prompt(item: dict) -> str:
        """从条目中抽取提示文本，按 prompt → question → answer 顺序回退。"""
        for key in ("prompt", "question", "answer"):
            v = item.get(key)
    ...
    try:
        ...
        # 校验第一条数据可提取提示
        if not _extract_prompt(data[0]):
            raise ValueError("数据格式错误：缺少可用提示字段（prompt/question/answer）")
        ...
        try:
            ...
            async def process_single_item(original_idx: int, item: dict):
                try:
                    ...
                    if not prompt:
                        content = "[ERROR] 缺少可用提示字段（prompt/question/answer）"
                        # 保存到临时文件并返回
                        temp_result = {
    ...
    try:
        ...
        error_responses = len([r for r in final_results if r[response_field_name].startswith('[ERROR]') or r[response_field_name].startswith('[EXCEPTION]')])
